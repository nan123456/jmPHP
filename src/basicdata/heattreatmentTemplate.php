...
//			$selectvalue = isset($_POST["selectvalue"]) ? json_decode($_POST["selectvalue"],TRUE) : array();
			...
			if(count($heattreatmentTableHeader) > 0){
				$sql  = "INSERT INTO `heattreatment_template`(weldingtree_id,model,productName,ownPartName,partsName,productDrawingNumber,ownPartDrawingNumber";
				...
				$sql2="select id from heattreatment_template where productDrawingNumber='".$heattreatmentTableHeader["productDrawingNumber"]."' and partsName='".$heattreatmentTableHeader["partsName"]."' order by id desc limit 1";
				$res2=$conn->query($sql2);
				$row2 =$res2->fetch_assoc();
				
				$sql3="INSERT INTO heattreatbody_template(heattreatment_id,model,temperature,time,otherdata,selectvalue)VALUES('".$row2["id"]."','$model','$temperature_string','$time_string','".$otherData_string."','".$selectvalue_string."')";
...
//			$selectvalue = isset($_POST["selectvalue"]) ? json_decode($_POST["selectvalue"],TRUE) : array();
			...
			//保存单一信息
			if(count($heattreatmentTableHeader) > 0){
				$sql  = "UPDATE heattreatment_template SET productName = '".$heattreatmentTableHeader["productName"]."',ownPartName = '".$heattreatmentTableHeader["ownPartName"]."',partsName = '".$heattreatmentTableHeader["partsName"]."',productDrawingNumber = '".$heattreatmentTableHeader["productDrawingNumber"]."',ownPartDrawingNumber = '".$heattreatmentTableHeader["ownPartDrawingNumber"]."',partsDrawingNumber = '".$heattreatmentTableHeader["partsDrawingNumber"]."' WHERE id = '".$contactId."'";
				$conn->query($sql);
...
//				$res2=$conn->query($sql2);
//				$row2 =$res2->fetch_assoc();
				$sql3="UPDATE heattreatbody_template SET temperature = '".$temperature_string."',time = '".$time_string."',otherdata = '".$otherData_string."',selectvalue = '".$selectvalue_string."'WHERE heattreatment_id = '".$contactId."'";
				$res3=$conn->query($sql3);
			}else{
				$returnData["state"] = "failure";
				$returnData["message"] = "主要数据为空";
		...
		case "getHeattreatmentInfoData":
			...
			//主表信息
			$sql = "SELECT * FROM `heattreatment_template` WHERE `id`='".$contactId."'";
			...
			$sql2="SELECT temperature,time,otherdata,selectvalue FROM `heattreatbody_template` WHERE `heattreatment_id`='".$contactId."'";
		...
		case "copyHeattreatment":
			...
			$sql = "select `weldingtree_id`,`ctime` from `heattreatment_template` where id='".$relateId."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$oldTime = $row["ctime"];
			//复制首表信息，返回自增id
			$sql = "INSERT INTO heattreatment_template ( weldingtree_id,model,productName,ownPartName,partsName,productDrawingNumber,ownPartDrawingNumber,partsDrawingNumber,ctime )";
			$sql .= "SELECT weldingtree_id,model,productName,ownPartName,partsName,productDrawingNumber,ownPartDrawingNumber,partsDrawingNumber,REPLACE(`ctime`,'".$oldTime."','".time()."') FROM heattreatment_template WHERE `id` = '".$relateId."'";
		...
		case "deleteHeattreatment":
			...
			//删除表头表尾
			$sql = "DELETE FROM `heattreatment_template` WHERE `id`='".$contactId."'";
			if(!$conn->query($sql)){
