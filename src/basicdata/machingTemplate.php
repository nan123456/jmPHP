	...
	switch ($flag) {
		case 'machingInserData':
			...
			if(count($machiningTableHeader) > 0){
				$sql = "INSERT INTO machiningtable_template ( craftsmanshiptree_id, hendnumber, productname, ownpartname, partname, workpiecenumber, productdrawnumber, ownpartdrawnumber, partdrawnumber, quantity, bottonimage, authorizedname, auditor, ctime,pnumber ) VALUES(";
				...
				if(!empty($autoIncrementId)){
					...
					foreach($machiningTableBody as $index => $datainfo){
						$sql = "INSERT INTO machingbody_template ( machingtable_id, serialnumber, process, workshop, processcontent, equipment, ctime ) VALUES(";
		...
		case 'machiningUpdateData':
			...
			if(count($_FILES) > 0){
				...
				if(isset($_FILES["myfileone"])){
					...
					$fileSaveSql_tmp = $uploadfileclass->uploadFile();
					$fileSaveSql = substr($fileSaveSql_tmp, 3);
					$sql = "UPDATE `machiningtable_template` SET `bottonimage`='".$fileSaveSql."' WHERE `id`='".$machiningTableHeader["contactId"]."'";
					$conn->query($sql);
				}
			...
			//保存信息
			if(count($machiningTableHeader) > 0){
				$sql = "UPDATE machiningtable_template SET hendnumber =  '".$machiningTableHeader["hendNumber"]."',productname = '".$machiningTableHeader["productName"]."',ownpartname = '".$machiningTableHeader["ownPartName"]."',partname = '".$machiningTableHeader["partsName"]."',workpiecenumber = '".$machiningTableHeader["workpieceNumber"]."',productdrawnumber = '".$machiningTableHeader["productDrawingNumber"]."',ownpartdrawnumber = '".$machiningTableHeader["ownPartDrawingNumber"]."',quantity = '".$machiningTableHeader["quantity"]."', bottonimage = '".$fileSaveSql."',authorizedname = '".$machiningTableFooter["name1"]."',auditor = '".$machiningTableFooter["name2"]."'WHERE id = '".$machiningTableHeader["contactId"]."'";
				$conn->query($sql);
				//获取machingbody内id
				$sql2 = "SELECT id FROM machingbody_template WHERE machingtable_id ='".$machiningTableHeader["contactId"]."'ORDER BY id";
				...
				//保存可遍历的信息
				foreach($machiningTableBody as $index => $datainfo){
					$sql = "UPDATE machingbody_template SET serialnumber = '".$datainfo["serialNumber"]."',process = '".$datainfo["processFlow"]."',workshop = '".$datainfo["workshop"]."',processcontent = '".$datainfo["skillsRequirement"]."',equipment = '".$datainfo["equipment"]."' WHERE id = '".$result2_arr[$j]."' ";
					$returnData["sql"]=$sql;
					$conn->query($sql);
					$j++;
					}
		...
		case 'getMachiningInfoData':
			...
			//主表信息
			$sql = "SELECT * FROM `machiningtable_template` WHERE `id`='".$contactId."'";
			...
			if($result->num_rows > 0){
				while($row = $result->fetch_assoc()){
					...
					//中间内容
					$sql = "SELECT * FROM `machingbody_template` WHERE `machingtable_id`='".$row["id"]."' ORDER BY `id`";
		...
		case 'copyMachining':
			...
			$sql = "select `craftsmanshiptree_id`,`ctime` from `machiningtable_template` where id='".$relateId."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$oldTime = $row["ctime"];
			//复制首表信息，返回自增id
			$sql = "INSERT INTO machiningtable_template ( craftsmanshiptree_id, hendnumber, productname, ownpartname, partname, workpiecenumber, productdrawnumber, ownpartdrawnumber, partdrawnumber, quantity, bottonimage, authorizedname, auditor, ctime )";
			$sql .= "select `craftsmanshiptree_id`,`hendnumber`,`productname`,`ownpartname`,`partname`,`workpiecenumber`,`productdrawnumber`,`ownpartdrawnumber`,`partdrawnumber`,`quantity`,`bottonimage`,`authorizedname`,`auditor`,REPLACE(`ctime`,'".$oldTime."','".time()."') from `machiningtable_template` where `id` = '".$relateId."'";
			$autoIncrementId = $conn->query($sql) ? $conn->insert_id : "";//获取成功插入后的id
		...
		case 'deleteMachining':
			...
			//删除表头表尾
			$sql = "DELETE FROM `machiningtable_template` WHERE `id`='".$contactId."'";
			if(!$conn->query($sql)){
				$returnData["state"] = "fail";
				$returnData["message"] .= "表头记录删除失败";
			}
			//删除内容
			$sql = "DELETE FROM `machingbody_template` WHERE `machingtable_id`='".$contactId."'";
			if(!$conn->query($sql)){
				$returnData["state"] = "fail";
