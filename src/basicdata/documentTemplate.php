...
//				echo $fileSaveName;
		...
		case "getTableListData" ://--------------查询焊接与制作工艺表格列表信息------------------
			...
			switch($tableFlag){
				case "1":
					//装载数据-焊接信息
					$sql = "SELECT `id` AS `contactId`,`productcode`,`processnumber`,`producname`,`partname`,workordernumber AS pnumber,FROM_UNIXTIME(`ctime`,'%Y-%m-%d %H:%i:%s') AS ctime,'welding' AS diff FROM `weldingtable_template` WHERE `weldingtree_id`='".$relateId."' order by `id` desc";
					$result = $conn->query($sql);
					if($result->num_rows > 0){
