...
//				echo $fileSaveName;
			...
			//保存首表信息，返回自增id
			$sql = "INSERT INTO `weldingtable_template`(`weldingtree_id`,`processnumber`,`quantity`,`workpiecenumber`,`workshop`,`workordernumber`,`producname`,`productcode`,`partname`,`partdrawingnumber`,`finallyresult`,`inspectorsingnature`,`finallydate`,`weldingsequence`,`weldingnumbermap`,`ctime`,weldingfoot,Organization,examine) VALUES(";
			...
			if(!empty($autoIncrementId)){
				...
				foreach($weldingtabletwo as $ky => $datainfo){
					$sql = "INSERT INTO `weldingtabletwo_template`(`weldingtable_id`,`serialnumber`,`checkcontent`,`processrequirement`,`testresult`,`singnature`,`ctime`) VALUES(";
				...
				foreach($weldingtablethree as $ky => $datainfo){
					$sql = "INSERT INTO `weldingtablethree_template`(`weldingtable_id`,`weldingnumber`,`requirementone`,`testresultone`,`singnatureone`,`requirementtwo`,`testresultonetwo`,`singnatureonetwo`,`requirementthree`,`testresultonethree`,`singnatureonethree`,`ctime`) VALUES(";
				...
				foreach($weldingtablefour as $ky => $datainfo){
					$sql = "INSERT INTO `weldingtablefour_template`(`weldingtable_id`,`weldingnumber`,`requirementone`,`testresultone`,`singnatureone`,`requirementtwo`,`testresultonetwo`,`singnatureonetwo`,`requirementthree`,`testresultonethree`,`singnatureonethree`,`ctime`) VALUES(";
					$sql .= "'".$autoIncrementId."','".$datainfo["weldNumber"]."','".$datainfo["processRequirements_1"]."','".$datainfo["testResult_1"]."','".$datainfo["inspectorSingnature_1"]."','".$datainfo["processRequirements_2"]."','".$datainfo["testResult_2"]."','".$datainfo["inspectorSingnature_2"]."','".$datainfo["processRequirements_3"]."','".$datainfo["testResult_3"]."','".$datainfo["inspectorSingnature_3"]."','".time()."')";
					$conn->query($sql);
				}
		...
		case "updateData" ://------------更新新的焊接数据------------------
			...
			if(count($_FILES) > 0){
				//删除原有文件
				$sql = "SELECT `weldingnumbermap` FROM `weldingtable_template` WHERE id='".$weldingtable["contactId"]."'";
				$result = $conn->query($sql);
				if($result->num_rows > 0){
			...
			//保存首表信息			
			$sql = "UPDATE `weldingtable_template` SET `processnumber`='".$weldingtable["processNumber"]."',`quantity`='".$weldingtable["quantity"]."',`workpiecenumber`='".$weldingtable["workpieceNumber"]."',`workshop`='".$weldingtable["workshop"]."'";
			...
			if(!empty($autoIncrementId)){
				//删除所有子表信息再重新保存
				$sql = "DELETE FROM `weldingtableone_template` WHERE `weldingtable_id`='".$autoIncrementId."';";
				$sql2 = "DELETE FROM `weldingtabletwo_template` WHERE `weldingtable_id`='".$autoIncrementId."';";
				$sql3 = "DELETE FROM `weldingtablethree_template` WHERE `weldingtable_id`='".$autoIncrementId."';";
				$sql4 = "DELETE FROM `weldingtablefour_template` WHERE `weldingtable_id`='".$autoIncrementId."';";
		...
		case "getTableListData" ://--------------查询焊接与制作工艺表格列表信息------------------
			...
			switch($tableFlag){
				case "1":
					//装载数据-焊接信息
					$sql = "SELECT `id` AS `contactId`,`productcode`,partdrawingnumber AS processnumber,`producname`,`partname`,workordernumber AS pnumber,FROM_UNIXTIME(`ctime`,'%Y-%m-%d %H:%i:%s') AS ctime,'welding' AS diff FROM `weldingtable_template` WHERE `weldingtree_id`='".$relateId."' order by `id` desc";
					$result = $conn->query($sql);
					if($result->num_rows > 0){
				...
				case "2":
					//装载数据-制造信息
					$sql = "SELECT `id` AS `contactId`,`productdrawnumber` AS `productcode`,`partdrawnumber` AS `processnumber`,productname AS `producname`,`partname`,pnumber,FROM_UNIXTIME(`ctime`,'%Y-%m-%d %H:%i:%s') AS ctime,'craftsmanship' AS diff FROM `craftsmanshiptable_template` WHERE `craftsmanshiptree_id`='".$relateId."' order by `id` desc";
					$result = $conn->query($sql);
					if($result->num_rows > 0){
						$returnData["message"] = "获取成功";
						$i = 0;
				...
				case "3":
					//装载数据-热处理
					$sql = "SELECT `id` AS `contactId`,partsDrawingNumber,productDrawingNumber,productName,ownPartName,partsName AS pnumber,FROM_UNIXTIME( `ctime`, '%Y-%m-%d %H:%i:%s' ) AS ctime,'heattreatment' AS diff FROM `heattreatment_template` WHERE	`weldingtree_id` = '".$relateId."' order by `id` desc";
					$result = $conn->query($sql);
					if($result->num_rows > 0){
						$returnData["message"] = "获取成功";
						$i = 0;
				...
				case "4":
					//装载数据-机加工
					$sql = "SELECT `id` AS `contactId`,`productdrawnumber` AS `productcode`,`partdrawnumber` AS `processnumber`,productname AS `producname`,`partname`,pnumber,FROM_UNIXTIME(`ctime`,'%Y-%m-%d %H:%i:%s') AS ctime,'machining' AS diff FROM `machiningtable_template` WHERE `craftsmanshiptree_id`='".$relateId."' order by `id` desc";
					$result = $conn->query($sql);
					if($result->num_rows > 0){
						$returnData["message"] = "获取成功";
						$i = 0;
				...
				case "0":
					...
					//装载数据-焊接信息
					$sql = "SELECT `id` AS `contactId`,`productcode`,`partdrawingnumber` AS productDrawingNumber,`producname` AS productName,`partname` AS ownPartName,workordernumber AS pnumber,FROM_UNIXTIME(`ctime`,'%Y-%m-%d %H:%i:%s') AS ctime,'welding' AS diff,'焊接工艺' AS TypeCard FROM `weldingtable_template` WHERE `weldingtree_id`='".$relateId[0]."' order by `id` desc";	
					$result = $conn->query($sql);
					$returnData["message"] = "获取成功";
					$i = 0;
					while($row = $result->fetch_assoc()){
					...
					//装载数据-热处理
					$sql3 = "SELECT `id` AS `contactId`,productDrawingNumber,productName,ownPartName,partsName AS pnumber,FROM_UNIXTIME( `ctime`, '%Y-%m-%d %H:%i:%s' ) AS ctime,'heattreatment' AS diff,'热处理' AS TypeCard FROM `heattreatment_template` WHERE	`weldingtree_id` = '".$relateId[0]."' order by `id` desc";
					...
					//装载数据-机加工
					$sql4 = "SELECT `id` AS `contactId`,productname AS `productName`,`partname` AS ownPartName,partdrawnumber AS productDrawingNumber,pnumber,FROM_UNIXTIME(`ctime`,'%Y-%m-%d %H:%i:%s') AS ctime,'machining' AS diff,'机械加工' AS TypeCard FROM `machiningtable_template` WHERE `craftsmanshiptree_id`='".$relateId[0]."' order by `id` desc";
		...
		case "getWeldingInfoData" ://----------------根据ID获取焊接相应的数据--------------------------
			...
			if(!empty($contactId)){
				//查询首表信息
				$sql = "SELECT `processnumber`,`quantity`,`workpiecenumber`,`workshop`,`workordernumber`,`producname`,`productcode`,`partname`,`partdrawingnumber`,`finallyresult`,`inspectorsingnature`,`finallydate`,`weldingsequence`,`weldingnumbermap`,weldingfoot,examine,Organization FROM `weldingtable_template` WHERE `id`='".$contactId."'";
				$result = $conn->query($sql);
				if($result->num_rows > 0){
					while($row = $result->fetch_assoc()){
				...
				//查询第二个表
				$sql = "SELECT `serialnumber`,`checkcontent`,`processrequirement`,`testresult`,`singnature` FROM `weldingtabletwo_template` WHERE `weldingtable_id`='".$contactId."' ORDER BY `id`";
				$result = $conn->query($sql);
				if($result->num_rows > 0){
					$i = 0;
					while($row = $result->fetch_assoc()){
				...
				//查询第三个表
				$sql = "SELECT `weldingnumber`,`requirementone`,`testresultone`,`singnatureone`,`requirementtwo`,`testresultonetwo`,`singnatureonetwo`,`requirementthree`,`testresultonethree`,`singnatureonethree` FROM `weldingtablethree_template` WHERE `weldingtable_id`='".$contactId."' ORDER BY `id`";
				$result = $conn->query($sql);
				if($result->num_rows > 0){
					$i = 0;
					while($row = $result->fetch_assoc()){
				...
				//查询第四个表
				$sql = "SELECT `weldingnumber`,`requirementone`,`testresultone`,`singnatureone`,`requirementtwo`,`testresultonetwo`,`singnatureonetwo`,`requirementthree`,`testresultonethree`,`singnatureonethree` FROM `weldingtablefour_template` WHERE `weldingtable_id`='".$contactId."' ORDER BY `id`";
				$result = $conn->query($sql);
				if($result->num_rows > 0){
					$i = 0;
					while($row = $result->fetch_assoc()){
		...
		case "DeleteWelding": //--------------------------------根据ID删除焊接信息----------------------------------------
			...
			$sql = "SELECT `weldingnumbermap` FROM `weldingtable_template` WHERE `id`='".$contactId."'";
			$result = $conn->query($sql);
			if($result->num_rows > 0){
				while($row = $result->fetch_assoc()){
			...
			$sql = "DELETE FROM `weldingtable_template` WHERE `id`='".$contactId."'";
			if(!$conn->query($sql)){
				$returnData["state"] = "fail";
				$returnData["message"] .= "表weldingtable记录删除失败；";
			}
			//删除表【weldingtableone】的数据
			$sql = "DELETE FROM `weldingtableone_template` WHERE `weldingtable_id`='".$contactId."'";
			if(!$conn->query($sql)){
				$returnData["state"] = "fail";
				$returnData["message"] .= "表weldingtableone记录删除失败；";
			}
			//删除表【weldingtabletwo】的数据
			$sql = "DELETE FROM `weldingtabletwo_template` WHERE `weldingtable_id`='".$contactId."'";
			if(!$conn->query($sql)){
				$returnData["state"] = "fail";
				$returnData["message"] .= "表weldingtabletwo记录删除失败；";
			}
			//删除表【weldingtablethree】的数据
			$sql = "DELETE FROM `weldingtablethree_template` WHERE `weldingtable_id`='".$contactId."'";
			if(!$conn->query($sql)){
				$returnData["state"] = "fail";
				$returnData["message"] .= "表weldingtablethree记录删除失败；";
			}
			//删除表【weldingtablefour】的数据
			$sql = "DELETE FROM `weldingtablefour_template` WHERE `weldingtable_id`='".$contactId."'";
			if(!$conn->query($sql)){
				$returnData["state"] = "fail";
		...
		case "craftsmanshipInsertDataOne" ://产品制作工艺技术要求及检验记录表信息保存【插入】【模板一】
			...
			if(count($craftsmanshipTableHeader) > 0){
				$sql = "INSERT INTO `craftsmanshiptable_template`(`craftsmanshiptree_id`,`model`,`productname`,`ownpartname`,`partname`,`workpiecenumber`,`productdrawnumber`,`ownpartdrawnumber`,`partdrawnumber`";
				$sql .= ",`quantity`,`finalconclusion`,`inspector`,`inspectionaudit`,`mark`,`numberofplaces`,`changethefilenumber`,`signature`,`date`,`establishment`,`review`,`conclusion`,`inconsistentconfirmation`,`firstfive`,`ctime`,`pnumber`) VALUES(";
				$sql .= "'".$treeId."','1'";
				...
				$sql .= ",'".$craftsmanshipTableBodyResult["conclusion"]."','".$craftsmanshipTableBodyResult["inconsistentConfirmation"]."','".$fileSaveSql."','".time()."','".$craftsmanshipTableHeader["pnumber"]."')";
				echo $sql;
				$autoIncrementId = $conn->query($sql) ? $conn->insert_id : "";//获取成功插入后的id
				...
				if(!empty($autoIncrementId)){
					...
					foreach($craftsmanshipTableBody_1 as $index => $datainfo){
						$sql = "INSERT INTO `craftsmanshiptableone_template`(`craftsmanship_id`,`serialnumber`,`processflow`,`inspectioncontent`,`skillsrequirement`,`selftest_13`,`selftest_14`,`selftest_15`,`selftest_16`";
		...
		case "craftsmanshipUpdateDataOne"://产品制作工艺技术要求及检验记录表信息保存【更新】【模板一】
			...
			if(count($craftsmanshipTableHeader) > 0){
				...
				$sql = "UPDATE `craftsmanshiptable_template` SET `productname`='".$craftsmanshipTableHeader["productName"]."',`ownpartname`='".$craftsmanshipTableHeader["ownPartName"]."',`partname`='".$craftsmanshipTableHeader["partsName"]."'";
				...
				if(!empty($autoIncrementId)){
					//先删除子表所有数据
					$sql = "DELETE FROM `craftsmanshiptableone_template` WHERE `craftsmanship_id`='".$autoIncrementId."'";
					if($conn->query($sql)){
						//保存可遍历的信息
						foreach($craftsmanshipTableBody_1 as $index => $datainfo){
							$sql = "INSERT INTO `craftsmanshiptableone_template`(`craftsmanship_id`,`serialnumber`,`processflow`,`inspectioncontent`,`skillsrequirement`,`selftest_13`,`selftest_14`,`selftest_15`,`selftest_16`";
			...
			if(count($craftsmanshipTableHeader) > 0){
				...
				if(!empty($autoIncrementId)){
					...
					foreach($craftsmanshipTableBody_2 as $index => $datainfo){
						$sql = "INSERT INTO `craftsmanshiptabletwo_template`(`craftsmanship_id`,`serialnumber`,`processflow`,`inspectioncontent`,`skillsrequirement`,`selftest`,`signature`,`ctime`) VALUES(";
						$sql .= "'".$autoIncrementId."','".$datainfo["serialNumber"]."','".$datainfo["processFlow"]."','".$datainfo["inspectionContent"]."','".$datainfo["skillsRequirement"]."'";
		...
		case "craftsmanshipUpdateDataTwo"://产品制作工艺技术要求及检验记录表信息保存【更新】【模板二】
			...
			if(count($_FILES) > 0){
				...
				if(isset($_FILES["myfileone"])){
					//删除原有文件
					$sql = "SELECT `secondmodelimageone` FROM `craftsmanshiptable_template` WHERE id='".$craftsmanshipTableHeader["contactId"]."'";
					$result = $conn->query($sql);
					if($result->num_rows > 0){
						while($row = $result->fetch_row()){
							$filePath = $row[0];
					...
					$fileSaveSql_tmp = $uploadfileclass->uploadFile();
					$fileSaveSql = substr($fileSaveSql_tmp, 3);
					$sql = "UPDATE `craftsmanshiptable_template` SET `secondmodelimageone`='".$fileSaveSql."' WHERE `id`='".$craftsmanshipTableHeader["contactId"]."'";
					$conn->query($sql);
				}
				//第二张
				$fileSaveName = getMillisecond().'_'.rand(100,999);//无后缀的文件名,毫秒级时间戳+3位随机数
				if(isset($_FILES["myfiletwo"])){
					//删除原有文件
					$sql = "SELECT `secondmodelimagetwo` FROM `craftsmanshiptable_template` WHERE id='".$craftsmanshipTableHeader["contactId"]."'";
					$result = $conn->query($sql);
					if($result->num_rows > 0){
						while($row = $result->fetch_row()){
							$filePath = $row[0];
					...
					$fileSaveSql_tmp = $uploadfileclass->uploadFile();
					$fileSaveSql = substr($fileSaveSql_tmp, 3);
					$sql = "UPDATE `craftsmanshiptable_template` SET `secondmodelimagetwo`='".$fileSaveSql."' WHERE `id`='".$craftsmanshipTableHeader["contactId"]."'";
					$conn->query($sql);
				}
				//第三张
				$fileSaveName = getMillisecond().'_'.rand(100,999);//无后缀的文件名,毫秒级时间戳+3位随机数
				if(isset($_FILES["myfilethree"])){
					//删除原有文件
					$sql = "SELECT `secondmodelimagethree` FROM `craftsmanshiptable_template` WHERE id='".$craftsmanshipTableHeader["contactId"]."'";
					$result = $conn->query($sql);
					if($result->num_rows > 0){
						while($row = $result->fetch_row()){
							$filePath = $row[0];
					...
					$fileSaveSql_tmp = $uploadfileclass->uploadFile();
					$fileSaveSql = substr($fileSaveSql_tmp, 3);
					$sql = "UPDATE `craftsmanshiptable_template` SET `secondmodelimagethree`='".$fileSaveSql."' WHERE `id`='".$craftsmanshipTableHeader["contactId"]."'";
					$conn->query($sql);
				}	
			}
			...
			if(count($craftsmanshipTableHeader) > 0){
				...
				if(!empty($autoIncrementId)){
					//先删除子表所有数据
					$sql = "DELETE FROM `craftsmanshiptabletwo_template` WHERE `craftsmanship_id`='".$autoIncrementId."'";
					if($conn->query($sql)){
						//保存可遍历的信息
						//保存可遍历的信息
						foreach($craftsmanshipTableBody_2 as $index => $datainfo){
		...
		case "craftsmanshipUpdateDataThree"://产品制作工艺技术要求及检验记录表信息保存【更新】【模板三】
			...
			if(count($_FILES) > 0){
				...
				if(isset($_FILES["myfile"])){
					//先删除
					$sql = "SELECT `thirdmodelimage` FROM `craftsmanshiptable_template` WHERE id='".$craftsmanshipTableHeader["contactId"]."'";
					$result = $conn->query($sql);
					if($result->num_rows > 0){
						while($row = $result->fetch_row()){
							$filePath = $row[0];
					...
					$fileSaveSql_tmp = $uploadfileclass->uploadFile();
					$fileSaveSql = substr($fileSaveSql_tmp, 3);
					$sql = "UPDATE `craftsmanshiptable_template` SET `thirdmodelimage`='".$fileSaveSql."' WHERE `id`='".$craftsmanshipTableHeader["contactId"]."'";
					$conn->query($sql);
				}
		...
		case "getCraftsmanshipInfoData"://获取制造工艺的详情信息【所有模板】
			...
			//主表信息
			$sql = "SELECT * FROM `craftsmanshiptable_template` WHERE `id`='".$contactId."'";
			$result = $conn->query($sql);
			if($result->num_rows > 0){
				while($row = $result->fetch_assoc()){
					...
					switch($row["model"]){//根据模型编号返回相应的信息
						case "1":														
							...
							//模板一可遍历的数据
							$sql = "SELECT * FROM `craftsmanshiptableone_template` WHERE `craftsmanship_id`='".$row["id"]."' ORDER BY `id`";
						...
						case "2":
							...
							//模板二可遍历的数据
							$sql = "SELECT * FROM `craftsmanshiptabletwo_template` WHERE `craftsmanship_id`='".$row["id"]."' ORDER BY `id`";
							$result_2 = $conn->query($sql);
							if($result_2->num_rows > 0){
								$i = 0;	
								while($row_2 = $result_2->fetch_assoc()){
		...
		case "deleteCraftsmanship": //----------------------------------根据【craftsmanshiptable】ID删除制造工艺信息--------------------------------------------------
			...
			$sql = "SELECT `secondmodelimageone`,`secondmodelimagetwo`,`secondmodelimagethree`,`thirdmodelimage` FROM `craftsmanshiptable_template` WHERE `id`='".$contactId."'";
			$result = $conn->query($sql);
			if($result->num_rows > 0){
				$savePathArr = array();
				while($row = $result->fetch_assoc()){
			...
			$sql = "DELETE FROM `craftsmanshiptable_template` WHERE `id`='".$contactId."' ";
			if(!$conn->query($sql)){
				$returnData["state"] = "fail";
				$returnData["message"] .= "表craftsmanshiptable记录删除失败";
			}
			//删除【craftsmanshiptableone】表记录
			$sql = "DELETE FROM `craftsmanshiptableone_template` WHERE `craftsmanship_id`='".$contactId."' ";
			if(!$conn->query($sql)){
				$returnData["state"] = "fail";
				$returnData["message"] .= "表craftsmanshiptableone记录删除失败";
			}
			//删除【craftsmanshiptabletwo】表记录
			$sql = "DELETE FROM `craftsmanshiptabletwo_template` WHERE `craftsmanship_id`='".$contactId."' ";
			if(!$conn->query($sql)){
				$returnData["state"] = "fail";
		...
		case "getWeldingPrintData"://----------------------------焊接工艺及检验记录打印--------------------------------------
			...
			//装载明细表数据
			$sql = "SELECT `id`,`processnumber`,`partname`,`partdrawingnumber` FROM weldingtable_template WHERE `weldingtree_id`='".$relateId."'";
			$result = $conn->query($sql);
			if($result->num_rows > 0){
				$i = 0;
				while($row = $result->fetch_assoc()){
			...
			foreach($weldingtableIdArr as $indexNum => $contactId){
				if(!empty($contactId)){
					//查询首表信息
					$sql = "SELECT `processnumber`,`quantity`,`workpiecenumber`,`workshop`,`workordernumber`,`producname`,`productcode`,`partname`,`partdrawingnumber`,`finallyresult`,`inspectorsingnature`,`finallydate`,`weldingsequence`,`weldingnumbermap` FROM `weldingtable_template` WHERE `id`='".$contactId."'";
					$result = $conn->query($sql);
					if($result->num_rows > 0){
						while($row = $result->fetch_assoc()){
		...
		case "getCraftsmanshipPrintData"://----------------------------------制造工艺及检验表打印------------------------------------------
			...
			//初始化数据形式
			$sql = "SELECT `id` FROM `craftsmanshiptable_template` WHERE `craftsmanshiptree_id`='".$relateId."'";
			$result = $conn->query($sql);
			if($result->num_rows > 0){
				$i = 0;
				while($row = $result->fetch_assoc()){
		...
		case "deleteTreeNodeWelding": //-------------------------------删除焊接树节点--------------------------------------------------
			...
			$sql = "SELECT `id` FROM `weldingtable_template` WHERE `weldingtree_id`='".$relateId."'";
			$result = $conn->query($sql);
			if($result->num_rows > 0){
				while($row = $result->fetch_assoc()){
					$contactIdArr[] = $row["id"];
		...
		case "copyWelding":
			...
			$sql = "select `weldingtree_id`,`ctime` from `weldingtable_template` where id='".$relateId."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$oldTime = $row["ctime"];
			//复制首表信息，返回自增id
			$sql = "INSERT INTO `weldingtable_template`(`weldingtree_id`,`processnumber`,`quantity`,`workpiecenumber`,`workshop`,`workordernumber`,`producname`,`productcode`,`partname`,`partdrawingnumber`,`finallyresult`,`inspectorsingnature`,`finallydate`,`weldingsequence`,`weldingnumbermap`,`ctime`)";
			$sql .= "select `weldingtree_id`,`processnumber`,`quantity`,`workpiecenumber`,`workshop`,`workordernumber`,`producname`,`productcode`,`partname`,`partdrawingnumber`,`finallyresult`,`inspectorsingnature`,`finallydate`,`weldingsequence`,`weldingnumbermap`,REPLACE(`ctime`,'".$oldTime."','".time()."') from `weldingtable_template` where `id` = '".$relateId."'";
			$autoIncrementId = $conn->query($sql) ? $conn->insert_id : "";//获取成功插入后的id
			...
			if(!empty($autoIncrementId)){
				...
				//复制第二个表信息
				$sql = "INSERT INTO `weldingtabletwo_template`(`weldingtable_id`,`serialnumber`,`checkcontent`,`processrequirement`,`testresult`,`singnature`,`ctime`)";
				$sql .= "select REPLACE(`weldingtable_id`,'".$relateId."','".$autoIncrementId."'),`serialnumber`,`checkcontent`,`processrequirement`,`testresult`,`singnature`,REPLACE(`ctime`,'".$oldTime."','".time()."') from `weldingtabletwo_template` where `weldingtable_id` = '".$relateId."'";
				$conn->query($sql);
				
				//复制第三个表信息
				$sql = "INSERT INTO `weldingtablethree_template`(`weldingtable_id`,`weldingnumber`,`requirementone`,`testresultone`,`singnatureone`,`requirementtwo`,`testresultonetwo`,`singnatureonetwo`,`requirementthree`,`testresultonethree`,`singnatureonethree`,`ctime`)";
				$sql .= "select REPLACE(`weldingtable_id`,'".$relateId."','".$autoIncrementId."'),`weldingnumber`,`requirementone`,`testresultone`,`singnatureone`,`requirementtwo`,`testresultonetwo`,`singnatureonetwo`,`requirementthree`,`testresultonethree`,`singnatureonethree`,REPLACE(`ctime`,'".$oldTime."','".time()."') from `weldingtablethree_template` where `weldingtable_id` = '".$relateId."'";
				$conn->query($sql);
				
				//复制第四个表信息
				$sql = "INSERT INTO `weldingtablefour_template`(`weldingtable_id`,`weldingnumber`,`requirementone`,`testresultone`,`singnatureone`,`requirementtwo`,`testresultonetwo`,`singnatureonetwo`,`requirementthree`,`testresultonethree`,`singnatureonethree`,`ctime`)";
				$sql .= "select REPLACE(`weldingtable_id`,'".$relateId."','".$autoIncrementId."'),`weldingnumber`,`requirementone`,`testresultone`,`singnatureone`,`requirementtwo`,`testresultonetwo`,`singnatureonetwo`,`requirementthree`,`testresultonethree`,`singnatureonethree`,REPLACE(`ctime`,'".$oldTime."','".time()."') from `weldingtablefour_template` where `weldingtable_id` = '".$relateId."'";
				$conn->query($sql);
				$returnData["message"] = "保存成功";
			}else{
				$returnData["state"] = "failure";
		...
		case "copyCraftsmanship":
			...
			//获取旧时间，复制修改函数REPLACE需要定值修改
			$sql = "select `craftsmanshiptree_id`,`ctime` from `craftsmanshiptable_template` where id='".$relateId."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$oldTime = $row["ctime"];
			//复制首表信息，也是模板三的表，返回自增id
			$sql = "INSERT INTO `craftsmanshiptable_template`(`craftsmanshiptree_id`,`model`,`productname`,`ownpartname`,`partname`,`workpiecenumber`,`productdrawnumber`,`ownpartdrawnumber`,`partdrawnumber`,`quantity`,`finalconclusion`,`inspector`,`inspectionaudit`,`mark`,`numberofplaces`,`changethefilenumber`,`signature`,`date`,`establishment`,`review`,`conclusion`,`inconsistentconfirmation`,`firstfive`,`ctime`)";
			$sql .= "select `craftsmanshiptree_id`,`model`,`productname`,`ownpartname`,`partname`,`workpiecenumber`,`productdrawnumber`,`ownpartdrawnumber`,`partdrawnumber`,`quantity`,`finalconclusion`,`inspector`,`inspectionaudit`,`mark`,`numberofplaces`,`changethefilenumber`,`signature`,`date`,`establishment`,`review`,`conclusion`,`inconsistentconfirmation`,`firstfive`,REPLACE(`ctime`,'".$oldTime."','".time()."') from `craftsmanshiptable_template` where `id` = '".$relateId."'";
			$autoIncrementId = $conn->query($sql) ? $conn->insert_id : "";//获取成功插入后的id
			
			if(!empty($autoIncrementId)){
				//复制模板一表信息
				$sql = "INSERT INTO `craftsmanshiptableone_template`(`craftsmanship_id`,`serialnumber`,`processflow`,`inspectioncontent`,`skillsrequirement`,`selftest_13`,`selftest_14`,`selftest_15`,`selftest_16`,`signature_1`,`qualityinspection_13`,`qualityinspection_14`,`qualityinspection_15`,`qualityinspection_16`,`signature_2`)";
				...
				//复制模板二表信息
				$sql = "INSERT INTO `craftsmanshiptabletwo_template`(`craftsmanship_id`,`serialnumber`,`processflow`,`inspectioncontent`,`skillsrequirement`,`selftest`,`signature`,`ctime`)";
				$sql .= "select REPLACE(`craftsmanship_id`,'".$relateId."','".$autoIncrementId."'),`serialnumber`,`processflow`,`inspectioncontent`,`skillsrequirement`,`selftest`,`signature`,REPLACE(`ctime`,'".$oldTime."','".time()."') from `craftsmanshiptabletwo_template` where `craftsmanship_id` = '".$relateId."'";
				$conn->query($sql);
				$returnData["message"] = "保存成功";
			}else{
				$returnData["state"] = "failure";
			...
			echo $json;
			break;
		case "getCopyTreeListData"://--------复制树节点信息-----------
		//接收数据
			...
			switch($type){
					...
					for($j=0;$j<16;$j++){
						...
						if($result->num_rows > 0){
							...
							while($row = $result->fetch_assoc()){
								...
								$returnData["data"][$j]["children"][$i]["thereId"] = $row["id"];
								$i++;
							}
						}
					}
				...
				case("craftsmanship"):
					...
					//查询大类树节点,0-15分别对应存货分类的16类
					for($j=0;$j<16;$j++){
						$sql = "SELECT `id`,`proname` FROM `craftsmanshiptree` where category='".$j."'";
						$result = $conn->query($sql);
						if($result->num_rows > 0){
							$i = 0;
							while($row = $result->fetch_assoc()){
		...
		case("getCreat"):
			...
			switch($cardtype){
				case("welding"):
					...
					//获取旧时间，复制修改函数REPLACE需要定值修改
					$sql4 = "select `weldingtree_id`,`ctime` from `weldingtable_template` where id='".$relateId."'";
					$result4 = $conn->query($sql4);
					...
					$result = $conn->query($sql);
					$row = $result->fetch_assoc();
					$sql2 = "SELECT partname,partdrawingnumber,weldingfoot,Organization,examine FROM weldingtable_template WHERE id='".$relateId."'";
					$result2 = $conn->query($sql2);
				...
				if(!empty($autoIncrementId)){
						...
						$sql .= "select REPLACE(`weldingtable_id`,'".$relateId."','".$autoIncrementId."'),`serialnumber`,`checkcontent`,`processrequirement`,`testresult`,`singnature`,REPLACE(`ctime`,'".$oldTime."','".time()."') from `weldingtabletwo_template` where `weldingtable_id` = '".$relateId."' ORDER BY `id`";
						$conn->query($sql);
				...
				case("craftsmanship"):
					...
					//获取旧时间，复制修改函数REPLACE需要定值修改
					$sql4 = "select `craftsmanshiptree_id`,`ctime` from `craftsmanshiptable_template` where id='".$relateId."'";
					$result4 = $conn->query($sql4);
					$row4 = $result4->fetch_assoc();
					$oldTime = $row4["ctime"];
					//查找要复制到的项目的首表信息
					$sql = 	"SELECT id,proname,procode,pnumber,REPLACE(`ctime`,'".$oldTime."','".time()."') as ctime FROM craftsmanshiptree WHERE id='".$type."'";
					$result = $conn->query($sql);
					$row = $result->fetch_assoc();
					$sql2 = "SELECT ownpartname,partname,partdrawnumber,model FROM craftsmanshiptable_template WHERE id='".$relateId."'";
					$result2 = $conn->query($sql2);
					$row2 = $result2->fetch_assoc();
					//复制首表信息，返回自增id
					$sql3 = "INSERT INTO `craftsmanshiptable`(`craftsmanshiptree_id`,`partname`,`ctime`,`pnumber`,`productname`,`productdrawnumber`,`ownpartname`,`partdrawnumber`,`model`) VALUES ('".$type."','".$row2["partname"]."','".time()."','".$row["pnumber"]."','".$row["proname"]."','".$row["procode"]."','".$row2["ownpartname"]."','".$row2["partdrawnumber"]."','".$row2["model"]."')";
					$autoIncrementId = $conn->query($sql3) ? $conn->insert_id : "";//获取成功插入后的id
					if(!empty($autoIncrementId)){
						//复制模板一表信息
				...
				case("heattreatment"):
					...
					//获取旧时间，复制修改函数REPLACE需要定值修改
					$sql4 = "select `weldingtree_id`,`ctime` from `heattreatment_template` where id='".$relateId."'";
					$result4 = $conn->query($sql4);
					$row4 = $result4->fetch_assoc();
					$oldTime = $row4["ctime"];
					//查找要复制到的项目的首表信息
					...
					$result = $conn->query($sql);
					$row = $result->fetch_assoc();
					$sql2 = "SELECT ownPartName,model,productDrawingNumber FROM heattreatment_template WHERE id='".$relateId."'";
					$result2 = $conn->query($sql2);
					$row2 = $result2->fetch_assoc();
					//复制首表信息，返回自增id
				...
				case("machining"):
					...
					//获取旧时间，复制修改函数REPLACE需要定值修改
					$sql4 = "select `craftsmanshiptree_id`,`ctime` from `machiningtable_template` where id='".$relateId."'";
					$result4 = $conn->query($sql4);
					$row4 = $result4->fetch_assoc();
					$oldTime = $row4["ctime"];
					//查找要复制到的项目的首表信息
					...
					$result = $conn->query($sql);
					$row = $result->fetch_assoc();
					$sql2 = "SELECT ownpartname,partname,partdrawnumber FROM machiningtable_template WHERE id='".$relateId."'";
					$result2 = $conn->query($sql2);
					$row2 = $result2->fetch_assoc();
					//复制首表信息，返回自增id
					$sql3 = "INSERT INTO `machiningtable`(`craftsmanshiptree_id`,`partname`,`ctime`,`pnumber`,`productname`,`productdrawnumber`,`ownpartname`,`partdrawnumber`) VALUES ('".$type."','".$row2["partname"]."','".time()."','".$row["pnumber"]."','".$row["proname"]."','".$row["procode"]."','".$row2["ownpartname"]."','".$row2["partdrawnumber"]."')";
					$autoIncrementId = $conn->query($sql3) ? $conn->insert_id : "";//获取成功插入后的id
					if(!empty($autoIncrementId)){
						$sql = "INSERT INTO machingbody ( machingtable_id, serialnumber, process, workshop, processcontent, equipment, ctime )";
						$sql .= "select REPLACE(`machingtable_id`,'".$relateId."','".$autoIncrementId."'),`serialnumber`,`process`,`workshop`,`processcontent`,`equipment`,REPLACE(`ctime`,'".$oldTime."','".time()."') from `machingbody_template` where `machingtable_id` = '".$relateId."' order by id";
						$conn->query($sql);
		...
		case("getCopy"):
			...
			switch($cardtype){
				case("welding"):
					...
					$row2 = $result2->fetch_assoc();
					//复制首表信息，返回自增id
					$sql3 = "INSERT INTO `weldingtable_template`(`weldingtree_id`,`partname`,`partdrawingnumber`,`ctime`,`workordernumber`,`producname`,`productcode`,weldingfoot,Organization,examine) VALUES ('".$type."','".$row2["partname"]."','".$row2["partdrawingnumber"]."','".time()."','".$row["pnumber"]."','".$row["proname"]."','".$row["procode"]."','".$row2["weldingfoot"]."','".$row2["Organization"]."','".$row2["examine"]."')";
					$autoIncrementId = $conn->query($sql3) ? $conn->insert_id : "";//获取成功插入后的id
				if(!empty($autoIncrementId)){
						//复制第一个表信息
						$sql = "INSERT INTO `weldingtableone_template`(`weldingtable_id`,`weldingnumber`,`materialfirst`,`specificationsfirst`,`materialsecond`,`specificationssecond`,`weldingmethod`,`grooveform`,`consumables`,`specifications`,`weldinglayer`,`weldingtrack`,`gas`,`current`,`actualcurrentfirst`,`actualcurrentsecond`,`voltage`,`actualvoltagefirst`,`actualvoltagesecond`,`specificationnumber`,`ratingnumber`,`flawdetection`,`steelstamp`,`ctime`)";
				...
				case("craftsmanship"):
					...
					$row2 = $result2->fetch_assoc();
					//复制首表信息，返回自增id
					$sql3 = "INSERT INTO `craftsmanshiptable_template`(`craftsmanshiptree_id`,`partname`,`ctime`,`pnumber`,`productname`,`productdrawnumber`,`ownpartname`,`partdrawnumber`,`model`) VALUES ('".$type."','".$row2["partname"]."','".time()."','".$row["pnumber"]."','".$row["proname"]."','".$row["procode"]."','".$row2["ownpartname"]."','".$row2["partdrawnumber"]."','".$row2["model"]."')";
					$autoIncrementId = $conn->query($sql3) ? $conn->insert_id : "";//获取成功插入后的id
					if(!empty($autoIncrementId)){
						//复制模板一表信息
						$sql = "INSERT INTO `craftsmanshiptableone_template`(`craftsmanship_id`,`serialnumber`,`processflow`,`inspectioncontent`,`skillsrequirement`,`selftest_13`,`selftest_14`,`selftest_15`,`selftest_16`,`signature_1`,`qualityinspection_13`,`qualityinspection_14`,`qualityinspection_15`,`qualityinspection_16`,`signature_2`)";
				...
				case("heattreatment"):
					...
					$row2 = $result2->fetch_assoc();
					//复制首表信息，返回自增id
					$sql3 = "INSERT INTO `heattreatment_template`(`weldingtree_id`,`model`,`ctime`,`productName`,`ownPartName`,`partsName`,`productDrawingNumber`) VALUES ('".$type."','".$row2["model"]."','".time()."','".$row["procode"].$row["proname"]."','".$row2["ownPartName"]."','".$row["procode"]."','".$row2["productDrawingNumber"]."')";
					$autoIncrementId = $conn->query($sql3) ? $conn->insert_id : "";//获取成功插入后的id
					if(!empty($autoIncrementId)){
						$sql = "INSERT INTO heattreatbody_template ( heattreatment_id, model, temperature, time, otherdata, selectvalue )";
						$sql .= "select REPLACE(`heattreatment_id`,'".$relateId."','".$autoIncrementId."'),`model`,`temperature`,`time`,`otherdata`,`selectvalue` from `heattreatbody_template` where `heattreatment_id` = '".$relateId."' order by id";
						$conn->query($sql);
						$returnData["message"] = "保存成功";
					}else{
				...
				case("machining"):
					...
					$row2 = $result2->fetch_assoc();
					//复制首表信息，返回自增id
					$sql3 = "INSERT INTO `machiningtable_template`(`craftsmanshiptree_id`,`partname`,`ctime`,`pnumber`,`productname`,`productdrawnumber`,`ownpartname`,`partdrawnumber`) VALUES ('".$type."','".$row2["partname"]."','".time()."','".$row["pnumber"]."','".$row["proname"]."','".$row["procode"]."','".$row2["ownpartname"]."','".$row2["partdrawnumber"]."')";
					$autoIncrementId = $conn->query($sql3) ? $conn->insert_id : "";//获取成功插入后的id
					if(!empty($autoIncrementId)){
						$sql = "INSERT INTO machingbody_template ( machingtable_id, serialnumber, process, workshop, processcontent, equipment, ctime )";
						$sql .= "select REPLACE(`machingtable_id`,'".$relateId."','".$autoIncrementId."'),`serialnumber`,`process`,`workshop`,`processcontent`,`equipment`,REPLACE(`ctime`,'".$oldTime."','".time()."') from `machingbody_template` where `machingtable_id` = '".$relateId."' order by id";
						$conn->query($sql);
		...
		case("getAllCopy"):
			...
			if($result1->num_rows > 0){
				while($row1 = $result1->fetch_assoc()){
					...
					$row2 = $result2->fetch_assoc();
					//复制首表信息，返回自增id
					$sql3 = "INSERT INTO `weldingtable_template`(`weldingtree_id`,`partname`,`partdrawingnumber`,`ctime`,`workordernumber`,`producname`,`productcode`,weldingfoot,Organization,examine) VALUES ('".$thereId[0]."','".$row2["partname"]."','".$row2["partdrawingnumber"]."','".time()."','".$row["pnumber"]."','".$row["proname"]."','".$row["procode"]."','".$row2["weldingfoot"]."','".$row2["Organization"]."','".$row2["examine"]."')";
					$autoIncrementId = $conn->query($sql3) ? $conn->insert_id : "";//获取成功插入后的id
					if(!empty($autoIncrementId)){
							//复制第一个表信息
							$sql = "INSERT INTO `weldingtableone_template`(`weldingtable_id`,`weldingnumber`,`materialfirst`,`specificationsfirst`,`materialsecond`,`specificationssecond`,`weldingmethod`,`grooveform`,`consumables`,`specifications`,`weldinglayer`,`weldingtrack`,`gas`,`current`,`actualcurrentfirst`,`actualcurrentsecond`,`voltage`,`actualvoltagefirst`,`actualvoltagesecond`,`specificationnumber`,`ratingnumber`,`flawdetection`,`steelstamp`,`ctime`)";
			...
			if($result1->num_rows > 0){
				while($row1 = $result1->fetch_assoc()){
					...
					$row = $result->fetch_assoc();
					$oldTime = $row["ctime"];
					$sql3 = "INSERT INTO `craftsmanshiptable_template`(`craftsmanshiptree_id`,`partname`,`ctime`,`pnumber`,`productname`,`productdrawnumber`,`ownpartname`,`partdrawnumber`,`model`) VALUES ('".$thereId[1]."','".$row2["partname"]."','".time()."','".$row4["pnumber"]."','".$row4["proname"]."','".$row4["procode"]."','".$row2["ownpartname"]."','".$row2["partdrawnumber"]."','".$row2["model"]."')";
					$autoIncrementId = $conn->query($sql3) ? $conn->insert_id : "";//获取成功插入后的id
					if(!empty($autoIncrementId)){
						//复制模板一表信息
						$sql = "INSERT INTO `craftsmanshiptableone_template`(`craftsmanship_id`,`serialnumber`,`processflow`,`inspectioncontent`,`skillsrequirement`,`selftest_13`,`selftest_14`,`selftest_15`,`selftest_16`,`signature_1`,`qualityinspection_13`,`qualityinspection_14`,`qualityinspection_15`,`qualityinspection_16`,`signature_2`)";
			...
			if($result1->num_rows > 0){
				while($row1 = $result1->fetch_assoc()){
					...
					$row2 = $result2->fetch_assoc();
					//复制首表信息，返回自增id
					$sql3 = "INSERT INTO `heattreatment_template`(`weldingtree_id`,`model`,`ctime`,`productName`,`ownPartName`,`partsName`,`productDrawingNumber`) VALUES ('".$thereId[0]."','".$row2["model"]."','".time()."','".$row["procode"].$row["proname"]."','".$row2["ownPartName"]."','".$row["pnumber"]."','".$row2["productDrawingNumber"]."')";
					$autoIncrementId = $conn->query($sql3) ? $conn->insert_id : "";//获取成功插入后的id
					if(!empty($autoIncrementId)){
						$sql = "INSERT INTO heattreatbody_template ( heattreatment_id, model, temperature, time, otherdata, selectvalue )";
			...
			if($result1->num_rows > 0){
				while($row1 = $result1->fetch_assoc()){
					...
					$row2 = $result2->fetch_assoc();
					//复制首表信息，返回自增id
					$sql3 = "INSERT INTO `machiningtable_template`(`craftsmanshiptree_id`,`partname`,`ctime`,`pnumber`,`productname`,`productdrawnumber`,`ownpartname`,`partdrawnumber`) VALUES ('".$thereId[0]."','".$row2["partname"]."','".time()."','".$row["pnumber"]."','".$row["proname"]."','".$row["procode"]."','".$row2["ownpartname"]."','".$row2["partdrawnumber"]."')";
					$autoIncrementId = $conn->query($sql3) ? $conn->insert_id : "";//获取成功插入后的id
					if(!empty($autoIncrementId)){
						$sql = "INSERT INTO machingbody_template ( machingtable_id, serialnumber, process, workshop, processcontent, equipment, ctime )";
