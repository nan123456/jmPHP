<?php 
  ...
  while($row = $result2->fetch_assoc()) {
    array_push($selectArr, array('product_name' => $row['product_name'], 'numbering' => $row['numbering'] , 'id' => $row['id'], 'installation_site' => $row['installation_site']));
  }
  ...
  foreach ($Arr as $key1 => $value1) {
    foreach ($selectArr as $key2 => $value2) {
      if($value1[1]==$value2['product_name'] && $value1[2]==$value2['numbering'] && $value1[4]==$value2['installation_site']) {
        // 重复数据
        array_push($delete, $value2['id']);
        break;
      }
    }
  }
  ...
  if($result1) {
    ...
    $sql2 = "INSERT INTO plan_table (serial_number,product_name,numbering,work_order,installation_site,exterior,colour,barrage,control_room,basic_embedded_parts,platform_automatic_door,car_body_cockpit,remarks,priority,frp_embedded_parts,scheduling_process_list,important_product,important_material_information,due_time,arrival_time,initial_planning_start_time,initial_plan_completion_time,actual_planned_start_time,actual_planned_completion_time,planned_scheduling_time,K,T_before_welding,T_welding,T_assembly,F,W,D_assembly,G,L_welding,IL_assembly,outer_track_column,in_plant_track_column,cockpit) VALUE ".$val2;
    $result2 = $conn->query($sql2);
    $json = '{"result":"'.$result2.'"}';
  }else {
    $json = '{"result":"'.false.'"}';
