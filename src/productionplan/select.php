...
//	header("Access-Control-Allow-Origin: *");
	...
	if ($flag == "Select") {
		...
		// 已就工数据列表
	  $sql4 = "select modid,fid,id,figure_number,name,standard,route,count,child_material,number,product_name,remark,routeid,backMark,reason,pNumber from productionplan WHERE isfinish='2' and route in ".$list." ORDER BY id DESC";
...
//	    $json = '{"success":true,"rows2":'.$list_data2.',"FStandard":'.$FStandard.',"FChild_material":'.$FChild_material.'}';
			...
			// 已完工数据列表
		  $sql4 = "select modid,fid,id,figure_number,name,standard,route,count,child_material,number,product_name,remark,routeid,backMark,reason,pNumber from productionplan WHERE isfinish='1' and route in $list ORDER BY id DESC,routeid";
		  $res4 = $conn->query($sql4);
		  if($res4->num_rows > 0 ){
		    $i = 0;
		    while($row4 = $res4->fetch_assoc()){
